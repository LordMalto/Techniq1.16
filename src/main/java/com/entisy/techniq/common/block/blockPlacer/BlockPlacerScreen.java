...
public class BlockPlacerScreen extends ContainerScreen<BlockPlacerContainer> {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(Techniq.MOD_ID,
            "textures/block/block_placer/gui.png");

    private final BlockPlacerContainer container;

    public BlockPlacerScreen(BlockPlacerContainer container, PlayerInventory inv, ITextComponent title) {
        ...
        if (mouseX >= getGuiLeft() + 154 && mouseX < getGuiLeft() + 154 + 12) {
            if (mouseY >= getGuiTop() + 18 && mouseY < getGuiTop() + 18 + 50) {
                ...
                this.renderTooltip(matrixStack,
                        new StringTextComponent(current.get() + "/" + BlockPlacerTileEntity.maxEnergy), mouseX, mouseY);
            }
        }
    }
}
