...
public class BlockBreakerScreen extends ContainerScreen<BlockBreakerContainer> {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(Techniq.MOD_ID,
            "textures/block/block_breaker/gui.png");

    private final BlockBreakerContainer container;

    public BlockBreakerScreen(BlockBreakerContainer container, PlayerInventory inv, ITextComponent title) {
        ...
        if (mouseX >= getGuiLeft() + 154 && mouseX < getGuiLeft() + 154 + 12) {
            if (mouseY >= getGuiTop() + 18 && mouseY < getGuiTop() + 18 + 50) {
                ...
                this.renderTooltip(matrixStack,
                        new StringTextComponent(current.get() + "/" + BlockBreakerTileEntity.maxEnergy), mouseX, mouseY);
            }
        }
    }
}
