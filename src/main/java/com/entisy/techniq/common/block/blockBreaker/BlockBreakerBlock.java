...
import com.entisy.techniq.common.block.SemiComplexBlock;
...
public class BlockBreakerBlock extends ComplexMachineBlock {
    ...
    public void setPlacedBy(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack item) {
        ...
        if (!item.getDisplayName().getString().isEmpty()) {
            ...
            if (tileEntity instanceof BlockBreakerTileEntity) {
                ((BlockBreakerTileEntity) tileEntity).setCustomName(item.getDisplayName());
            }
        }
    }
    ...
    public void onRemove(BlockState state, World world, BlockPos pos, BlockState newState, boolean isMoving) {
        TileEntity tileEntity = world.getBlockEntity(pos);
        if (tileEntity instanceof BlockBreakerTileEntity && state.getBlock() != newState.getBlock()) {
            BlockBreakerTileEntity furnace = (BlockBreakerTileEntity) tileEntity;
