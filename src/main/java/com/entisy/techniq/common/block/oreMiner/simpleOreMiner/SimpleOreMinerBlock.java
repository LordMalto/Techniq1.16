...
public class SimpleOreMinerBlock extends MachineBlock {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return ModTileEntityTypes.SIMPLE_ORE_MINER_TILE_ENTITY.get().create();
    }
    ...
    public void setPlacedBy(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack item) {
        ...
        if (!item.getDisplayName().getString().isEmpty()) {
            ...
            if (tileEntity instanceof SimpleOreMinerTileEntity) {
                ((SimpleOreMinerTileEntity) tileEntity).setCustomName(item.getDisplayName());
            }
        }
    }
    ...
    public void onRemove(BlockState state, World world, BlockPos pos, BlockState newState, boolean isMoving) {
        TileEntity tileEntity = world.getBlockEntity(pos);
        if (tileEntity instanceof SimpleOreMinerTileEntity && state.getBlock() != newState.getBlock()) {
            SimpleOreMinerTileEntity furnace = (SimpleOreMinerTileEntity) tileEntity;
