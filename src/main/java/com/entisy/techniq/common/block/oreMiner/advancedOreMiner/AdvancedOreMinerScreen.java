...
public class AdvancedOreMinerScreen extends ContainerScreen<AdvancedOreMinerContainer> {
    ...
    public AdvancedOreMinerScreen(AdvancedOreMinerContainer container, PlayerInventory inv, ITextComponent title) {
    ...
    protected void renderTooltip(MatrixStack matrixStack, int mouseX, int mouseY) {
        ...
        if (mouseX >= getGuiLeft() + 154 && mouseX < getGuiLeft() + 154 + 12) {
            if (mouseY >= getGuiTop() + 18 && mouseY < getGuiTop() + 18 + 50) {
                ...
                this.renderTooltip(matrixStack,
                        new StringTextComponent(current.get() + "/" + AdvancedOreMinerTileEntity.maxEnergy), mouseX, mouseY);
            }
        }
        if (mouseX >= getGuiLeft() + 66 && mouseX < getGuiLeft() + 66 + 4) {
            if (mouseY >= getGuiTop() + 35 && mouseY < getGuiTop() + 35 + 16) {
                ...
                int current = 0;
                if (!getMenu().tileEntity.isBlockNull()) current = (int) (getMenu().tileEntity.currentSmeltTime * 100 / getMenu().tileEntity.getMiningTime());
