...
public class RefineryBlock extends ComplexMachineBlock {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return ModTileEntityTypes.REFINERY_TILE_ENTITY.get().create();
    }
    ...
    public void setPlacedBy(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack item) {
        ...
        if (!item.getDisplayName().getString().isEmpty()) {
            ...
            if (tileEntity instanceof RefineryTileEntity) {
                ((RefineryTileEntity) tileEntity).setCustomName(item.getDisplayName());
            }
        }
    }
    ...
    public void onRemove(BlockState state, World world, BlockPos pos, BlockState newState, boolean isMoving) {
        TileEntity tileEntity = world.getBlockEntity(pos);
        if (tileEntity instanceof RefineryTileEntity && state.getBlock() != newState.getBlock()) {
            RefineryTileEntity furnace = (RefineryTileEntity) tileEntity;
