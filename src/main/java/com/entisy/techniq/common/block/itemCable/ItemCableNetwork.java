...
public class ItemCableNetwork implements IEnergyStorage {
    ...
    private ItemCableNetwork(IWorldReader world, Set<BlockPos> wires, int energyStored) {
        this.world = world;
    ...
    private void updateWireEnergy() {
        ...
        connections.keySet().forEach(p -> {
            ...
            if (tileEntity instanceof ItemCableTileEntity) {
                ((ItemCableTileEntity) tileEntity).energyStored = energyPerWire;
            }
        });
    }
...
//        }
    ...
    static ItemCableNetwork buildNetwork(IWorldReader world, BlockPos pos) {
        ...
        int energyStored = wires.stream().mapToInt(p -> {
            TileEntity tileEntity = world.getBlockEntity(p);
            return tileEntity instanceof ItemCableTileEntity ? ((ItemCableTileEntity) tileEntity).energyStored : 0;
        }).sum();
        return new ItemCableNetwork(world, wires, energyStored);
    }
    ...
    private Set<Connection> getConnections(IBlockReader world, BlockPos pos) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (te != null && !(te instanceof ItemCableTileEntity) && te.getCapability(CapabilityEnergy.ENERGY).isPresent()) {
